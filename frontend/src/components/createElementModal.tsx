...
export default function CreateElementModal({application, fields, onClose }: CreateElementModalProps) {
    const { addToast } = useToast();
    const queryClient = useQueryClient();

    const mutation = useMutation({
        mutationFn: (newData: Record<string, any>) => {
            return api.post(`/${application}`, newData).then(res=>res.data);
        },

        onSuccess: () => {
            addToast(`${application.charAt(0).toUpperCase() + application.slice(1)} created successfully.`, 'success', 5000);
            queryClient.invalidateQueries({ queryKey: [application]})
            onClose();
        },

        onError: (error) => {
            addToast(`Ran into an error creating ${application}. Please try again.`, 'error', 10000);
            console.error(error);
        }
    })
